...
{
    public class InfrastructurePortService : BasePortService, IInfrastructurePortService
    {
        private readonly IPortDirector _director;

        public InfrastructurePortService(IUnitOfWork unitOfWork, IMapper mapper, 
        ...
        {
            return base.GetAllByAttachmentIDAsync(attachmentId, isProviderDomainRole: true, deep: deep, asTrackable: asTrackable);
        }

        public Task<IEnumerable<Port>> GetAllByDeviceIDAsync(int deviceId, bool? deep = false, bool asTrackable = false)
        {
            return base.GetAllByDeviceIDAsync(deviceId, isProviderDomainRole: true, deep: deep, asTrackable: asTrackable);
        }

        public Task<IEnumerable<Port>> GetAllByInterfaceIDAsync(int interfaceId, bool? deep = false, bool asTrackable = false)
        {
            return base.GetAllByInterfaceIDAsync(interfaceId, isProviderDomainRole: true, deep: deep, asTrackable: asTrackable);
        }

        public Task<Port> GetByIDAsync(int id, bool? deep = false, bool asTrackable = false)
        {
            return base.GetByIDAsync(id, isProviderDomainRole: true, deep: deep, asTrackable: asTrackable);
        }
