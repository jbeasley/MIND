...
${nuget_cmd} install src/IO.NovaAttSwagger/packages.config -o packages;

echo "[INFO] Copy DLLs to the 'bin' folder"
mkdir -p bin;
cp packages/Newtonsoft.Json.10.0.3/lib/net45/Newtonsoft.Json.dll bin/Newtonsoft.Json.dll;
cp packages/RestSharp.106.5.3/lib/net452/RestSharp.dll bin/RestSharp.dll;
cp packages/JsonSubTypes.1.2.0/lib/net45/JsonSubTypes.dll bin/JsonSubTypes.dll

echo "[INFO] Run 'mcs' to build bin/IO.NovaAttSwagger.dll"
mcs -langversion:${langversion} -sdk:${sdk} -r:bin/Newtonsoft.Json.dll,bin/JsonSubTypes.dll,\
bin/RestSharp.dll,\
System.ComponentModel.DataAnnotations.dll,\
System.Runtime.Serialization.dll \
-target:library \
-out:bin/IO.NovaAttSwagger.dll \
-recurse:'src/IO.NovaAttSwagger/*.cs' \
-doc:bin/IO.NovaAttSwagger.xml \
-platform:anycpu
...
then
  ...
  echo "[INFO] bin/IO.NovaAttSwagger.dll was created successfully"
fi
